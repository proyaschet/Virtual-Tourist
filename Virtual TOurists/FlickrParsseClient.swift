...
class FlickerClient
{
    ...
    {
        let request = NSMutableURLRequest(url: flickrURLBuilder(parameters, withPathExtension: nil))
    ...
    func getPhotosUsingFlickr(_ pin: Pin?, completionHandlerForGETPhotosUsingFlickr: @escaping (_ success: Bool, _ errorString: String?) -> Void) {
        ...
        let _ = sessionTaskGetMethod(parameters: parameters!) { (results, error) in
    ...
    func getPhotosUsingPage(_ pin: Pin?, randomPage: Int?, perPage: Int?, completionHandlerForGETPhotosUsingPage: @escaping (_ success: Bool, _ errorString: String?) -> Void) {
